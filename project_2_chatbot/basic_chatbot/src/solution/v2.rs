...
impl ChatbotV2 { //putting functions into the objects in struct
    #[allow(dead_code)]
    pub fn new(model: Llama) -> ChatbotV2 { //executes when launched for first time to constuct chatbot

    let chat_session = model //selects the model and starts the chat session during model initialisation
            .chat()
